...
// src/locales/<locale>/views/admin/ExperienceListView.json
const t = useT("views.admin.ExperienceListView");
...
// * Id experience yang lagi diproses hapus SAAT INI - dipake buat (1)
// nge-guard biar handleDelete/handleBulkDelete gak bisa di-trigger ulang buat
// item yang sama selagi masih diproses (klik ganda), (2) disable tombol
// delete per-card di AdminExperienceCard.vue (deletingIds.has(experience.id)),
// dan (3) nentuin kapan floating progress indicator di bawah nongol. Diisi
// baik dari handleDelete (single) maupun handleBulkDelete (banyak sekaligus).
...
async function handleDelete(experience: Experience) {
  // * Guard: kalau experience ini udah lagi diproses hapus (single ATAU lagi
  // ikut ke-include di bulk delete yang jalan), abaikan klik berikutnya.
  // Tombol delete di card-nya sendiri juga udah di-disable (lihat template) -
  // ini lapisan kedua jaga-jaga (mis. Enter key event / race klik cepat).
  if (deletingIds.value.has(experience.id)) return;

  const confirmed = await confirm({
    title: t("confirmDeleteTitle"),
    message: t("confirmDelete", { name: experience.position }),
  ...
  deletingIds.value.add(experience.id);
  try {
    ...
    deletingIds.value.delete(experience.id);
  }
}
  ...
  <div v-motion="fadeUp()">
    ...
    <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
      <AdminExperienceCard
        ...
        :deleting="deletingIds.has(experience.id)"
