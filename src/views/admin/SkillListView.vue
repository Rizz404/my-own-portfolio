...
import { Plus as IconPlus, Trash2 as IconTrash2, ListChecks as IconListChecks, Search as IconSearch, Sparkles as IconSparkles, Loader2 as IconLoader } from "@lucide/vue";
import AppButton from "@/components/shared/AppButton.vue";
import AppSelect from "@/components/shared/AppSelect.vue";
import AppCheckbox from "@/components/shared/AppCheckbox.vue";
import AppSkeleton from "@/components/shared/AppSkeleton.vue";
import AppError from "@/components/shared/AppError.vue";
import AdminSkillCard from "@/components/admin/AdminSkillCard.vue";
import { useSkillsQuery, useDeleteSkillMutation } from "@/composables/queries/useSkills";
...
// src/locales/<locale>/views/admin/SkillListView.json
const t = useT("views.admin.SkillListView");
...
// useQuerySync.ts.
const queryParams = ref<SkillQueryParams>({
  page: 1,
  size: 10,
  search: "",
  category: undefined,
...
const { data: skillResponse, isLoading, isError, isFetching, error } = useSkillsQuery(queryParams);
...
// * Value opsi filter/category pakai nama key enum-nya ("framework", "database", dst)
// biar cocok sama `SkillQueryParams.category: string` & format yang dibalikin backend.
const categoryOptions = computed(() => [
  { label: t("filters.allCategories"), value: "" },
  ...enumStringKeys(SkillCategory).map((key) => ({ label: formatLabel(key), value: key })),
]);
...
function applyPage() {
  const totalPages = skillResponse.value?.pagination.totalPages ?? 1;
...
const prevPage = () => {
  if (skillResponse.value?.pagination.hasPrevPage) queryParams.value.page!--;
};

const nextPage = () => {
  if (skillResponse.value?.pagination.hasNextPage) queryParams.value.page!++;
};
...
// * Id skill yang lagi diproses hapus SAAT INI - dipake buat (1) nge-guard
// biar handleDelete/handleBulkDelete gak bisa di-trigger ulang buat item yang
// sama selagi masih diproses (klik ganda), (2) disable tombol delete
// per-card di AdminSkillCard.vue (deletingIds.has(skill.id)), dan (3)
// nentuin kapan floating progress indicator di bawah nongol. Diisi baik dari
// handleDelete (single) maupun handleBulkDelete (banyak sekaligus).
...
const isAllSelectedOnPage = computed(() => {
  const items = skillResponse.value?.data ?? [];
  return items.length > 0 && items.every((skill) => selectedIds.value.has(skill.id));
});
...
function toggleSelectAllOnPage() {
  ...
  if (isAllSelectedOnPage.value) {
    items.forEach((skill) => selectedIds.value.delete(skill.id));
  } else {
    items.forEach((skill) => selectedIds.value.add(skill.id));
  }
}
...
async function handleDelete(skill: Skill) {
  // * Guard: kalau skill ini udah lagi diproses hapus (single ATAU lagi ikut
  ...
  if (deletingIds.value.has(skill.id)) return;

  const confirmed = await confirm({
    title: t("confirmDeleteTitle"),
    message: t("confirmDelete", { name: skill.name }),
  ...
  deletingIds.value.add(skill.id);
  try {
    ...
    deletingIds.value.delete(skill.id);
  }
}
...
<template>
  <div v-motion="fadeUp()">
    <div class="flex flex-col w-full gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
      ...
      <RouterLink :to="{ name: 'AdminSkillCreate' }">
        <AppButton>
    ...
    <div v-if="skillResponse?.data.length" class="flex flex-wrap items-center justify-between gap-3 mb-4">
    ...
    <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
      <AdminSkillCard
        v-for="(skill, index) in skillResponse?.data"
        :key="skill.id"
        ...
        @delete="handleDelete"
        @update:selected="(value) => toggleSelect(skill.id, value)"
      />
    </div>

    <div v-if="skillResponse?.data.length" class="flex flex-wrap items-center justify-center gap-4 mt-8">
      <AppButton
        variant="secondary"
        size="sm"
        :disabled="!skillResponse?.pagination.hasPrevPage || isFetching"
        @click="prevPage"
      >
      ...
      <div class="flex items-center gap-2 text-sm text-content/70">
        ...
        <input
          ...
          type="number"
          min="1"
          :max="skillResponse?.pagination.totalPages ?? 1"
          ...
          @change="applyPage"
        />
        <span>{{ t("pagination.of") }} {{ skillResponse?.pagination.totalPages ?? 1 }}</span>
      </div>

      <AppButton
        variant="secondary"
        size="sm"
        :disabled="!skillResponse?.pagination.hasNextPage || isFetching"
        @click="nextPage"
      >
      ...
      <span class="text-sm text-content/50">
        {{ t("pagination.total", { count: skillResponse?.pagination.total ?? 0 }) }}
      </span>
    </div>
