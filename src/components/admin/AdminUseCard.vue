...
import { Package as IconPackage, Trash2 as IconTrash2, Loader2 as IconLoader, Calendar as IconCalendar, Check as IconCheck } from "@lucide/vue";
import type Use from "@/types/use";
import { formatDate } from "@/utils/dateUtil";
import { useT } from "@/composables/useT";

// * Namespace translation buat komponen ini, ikutin path file JSON-nya:
// src/locales/<locale>/components/admin/AdminUseCard.json
const t = useT("components.admin.AdminUseCard");

const props = defineProps<{
  use: Use;
  deleting?: boolean;
  selected?: boolean;
  // * Mode seleksi di-toggle dari UseListView.vue. Gak ada checkbox terpisah -
...
function handleCardClick() {
  if (props.selectable) {
    ...
    router.push({ name: "AdminUseEdit", params: { id: props.use.id } });
  }
}
...
<template>
  <div
    ...
    role="button"
    :aria-label="selectable ? (selected ? t('deselect') : t('select')) : use.itemName"
  ...
  >
    <div class="flex items-start justify-between gap-3">
      <div class="flex items-center flex-1 min-w-0 gap-3">
        ...
        <div class="min-w-0">
          <p class="font-semibold truncate text-content">{{ use.itemName }}</p>
          <p class="text-xs truncate text-content/50">{{ formatLabel(use.category) }}</p>
        </div>
      </div>
    ...
    <p v-if="use.reasons" class="text-sm leading-relaxed text-content/60 line-clamp-2">
      {{ use.reasons }}
    </p>
    ...
    <div class="flex items-center gap-1.5 pt-3 mt-auto text-xs border-t border-border/50 text-content/50">
      ...
      {{ t("updated") }} {{ formatDate(use.updatedAt) }}
    </div>
  </div>
</template>
